    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3" noValidate>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          ...
          <ImageUploader value={watch("imageUrl") ?? ""} onChange={(url) => setValue("imageUrl", url)} />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label>Audio phát âm (không bắt buộc)</Label>
          <QuestionAudioUploader value={watch("audioUrl") ?? ""} onChange={(url) => setValue("audioUrl", url)} />
        </div>
      </div>
