    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
            <button
              type="button"
            ...
            <div className="flex flex-1 flex-col gap-1">
              <Input {...register(`options.${index}.content` as const)} className="bg-muted/40" />
              <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Lựa chọn {OPTION_LABELS[index]}</span>
            </div>
            {fields.length > 2 && (
              <Button type="button" variant="ghost" size="icon" onClick={() => remove(index)} aria-label={`Xóa lựa chọn ${OPTION_LABELS[index]}`}>
