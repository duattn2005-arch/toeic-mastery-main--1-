...
// Quizlet-style card row used by the bulk vocabulary editor — term/definition
// are required to save, image/audio are optional per-card media.
export const vocabularyWordRowSchema = z.object({
  term: z.string().trim().min(1),
  definition: z.string().trim().min(1),
  imageUrl: z.string().trim().optional(),
  audioUrl: z.string().trim().optional(),
});
export type VocabularyWordRowInput = z.infer<typeof vocabularyWordRowSchema>;
