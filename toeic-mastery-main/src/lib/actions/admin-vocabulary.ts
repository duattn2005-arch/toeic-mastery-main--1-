...
export async function bulkCreateVocabularyWordsAction(
  topicId: string,
  rows: { term: string; definition: string; imageUrl?: string; audioUrl?: string }[]
): Promise<ActionResult & { count?: number }> {
  await requireAdmin();
  if (!topicId) return { error: "Vui lòng chọn chủ đề" };

  const clean = rows
    .map((r) => ({ term: r.term.trim(), definition: r.definition.trim(), imageUrl: r.imageUrl?.trim(), audioUrl: r.audioUrl?.trim() }))
  ...
  const result = await db.vocabularyWord.createMany({
    data: clean.map((r) => ({
      topicId,
      word: r.term,
      meaningVi: r.definition,
      imageUrl: r.imageUrl || null,
      audioUrlUs: r.audioUrl || null,
    })),
    skipDuplicates: true,
  });
