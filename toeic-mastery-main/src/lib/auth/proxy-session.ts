...
// app-wide login gate: a logged-out visitor lands on the real page with an
// inline "log in to continue" prompt (see LoginRequiredGate) instead of
// being redirected here. Exact-pathname match, so nested routes like
// /practice/[testId] and /practice/mistakes are unaffected and still
// require a session.
const PUBLIC_ROUTES = ["/", "/practice", "/practice/start", ...AUTH_ROUTES];
